...
class MolEditFormer_finetune(nn.Module):
    ...
    def forward(self, batch_input_molecule, batch_input_text, batch_output_molecule, batch_output_text=None):
        ...
        if self.mol_args.molecule_type in ["SMILES", "all"]:
            ...
            molecule_latent = self.mol2latent(output_molecule_repr)
        ...
        cl_loss, mask_loss = self._calc_finetune_loss(molecule_latent=molecule_latent, text_latent=text_latent, target_token_ids=output_molecule_token_ids[1:, :], target_mask=output_molecule_mask[1:, :], token_output=token_output)

        return cl_loss, mask_loss

    def _calc_finetune_loss(self, molecule_latent, text_latent, target_token_ids, target_mask, token_output):
