...
def main(args):
    ...
    logger = Logger(args, args.store_dir, args.time_log)
    model_save_name = f"{args.molecule_type}_{args.gnn_type}-{get_local_time()}-alignment.pkl"
    ...
    if args.repr_frozen:
        ...
        model_param_group = [
            ...
            {"params": model.mol2latent.parameters(), "lr": args.mol_lr * args.mol_lr_scale},
        ]
    else:
        model_param_group = [
        {"params": model.text_model.parameters(), "lr": args.text_lr},
        {"params": model.molecule_model.parameters(), "lr": args.mol_lr},
        {"params": model.text2latent.parameters(), "lr": args.text_lr * args.text_lr_scale},
        {"params": model.mol2latent.parameters(), "lr": args.mol_lr * args.mol_lr_scale},
    ]
    ...
    for epoch_id in range(args.epoch_num):
        ...
        for i_batch, sample_batched in tqdm(enumerate(train_loader), disable=False, total=len(train_loader)):
            ...
            grpah_batched = sample_batched[2].to(device)
            ...
            image_features, text_features = model(
                grpah_batched, description_batched, device
            )
            ...
            if i_batch % 100 == 0:
                logger.log("{}th loss in {} epoch:{}".format(i_batch, epoch_id + 1, loss))
            epoch_loss += loss / len(train_loader)
        # break
        logger.log("{}th epoch mean loss:{}".format(epoch_id + 1, epoch_loss))
    torch.save(model.state_dict(), osp.join(args.save_dir, model_save_name))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mol_lr", type=float, default=2e-5)
    ...
    # load config
    parser.add_argument('--text_pretrain_folder', type=str, default='ckpt/SciBERT')
    parser.add_argument('--graph_pretrain_folder', type=str, default='ckpt/GraphMVP')
    # save config
